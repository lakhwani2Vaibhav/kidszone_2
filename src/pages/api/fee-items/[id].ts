  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (result.matchedCount) {
        const feeItem = await feeItemsCollection.findOne({ _id: new ObjectId(id as string) });
        res.json(serializeDoc(feeItem));
      } else {
        res.status(404).json({ error: 'Fee item not found' });
      }
    } else if (req.method === 'DELETE') {
      const result = await feeItemsCollection.deleteOne({ _id: new ObjectId(id as string) });
    ...
    console.error('Fee item API error:', error);
